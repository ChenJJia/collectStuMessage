		...
		if(!result){
			...
			if(textInput[0].value == textInput[2].value){
				...
				if(textP[2].style.display != "家长姓名与学生姓名不能一致")
     			...
     			textP[0].style.color = "#b3b3b3";
				textP[0].innerHTML = textInput[0].value+"同学，您好，请核对您的信息是否有误: </br>学号: </br>班级: <br />宿舍号： </br>毕业中学: </br>如信息错误请检查姓名是否输入有误;</br>如信息核对正确，请您完善您的个人相关信息;";
...
//		checkLi[5].innerHTML = "家庭住址: "+textInput[4].value+textInput[5].value;
	...
	succeedCertain[0].onclick = function () {
		...
		textForm.style.visibility = "visible";
		shade.style.display = "none";
		window.location.href="https://508cst.gcu.edu.cn/zeroone/more/more.html";
	}
		...
		success:function(checkMess){ //message表示后台返回的数据
			...
			if(checkMess.status == "accept"){
				...
				if(checkMess.list==null){
					...
					textP[0].innerHTML =textInput+"同学，您好，请核对您的信息是否有误: </br>学号: "+checkMess.stuNo+"</br>班级: "+checkMess.classname+"<br />宿舍号："+checkMess.dormitory+"</br>毕业中学: "+checkMess.highschool+"</br>如信息错误请检查姓名是否输入有误;</br>如信息核对正确，请您完善您的个人相关信息;";
				}
				else{
					...
					sameIdBtn[1].onclick = function(){
						...
						for(var i=0;i<checkMess.list.length;i++){
							if(sameIdTd[3*(i+1)-1].style.backgroundImage=="url(\"res/front/img/icon_locked_03.png\")"){
