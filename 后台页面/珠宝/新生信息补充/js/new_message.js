...
function myFunction1(x) {
	if(x.value == "请输入学生姓名"){
		x.value = "";
		x.style.borderBottom = "1px solid #444";
	}
	else if(x.value == ""){
		x.value = "请输入学生姓名";
...
function myFunctionOnblur(x){
	...
	if(x.value == "请输入学生姓名"){
		...
		if(!re.test(textInput[0].value)&&textInput[0].value.toString().length>1){
			studentName(textInput[0].value,college.innerHTML);
		}			
	}
...
function submit(){
	...
	&& textInput[1].value[0]==1 && textInput[3].value[0]==1
	&& textInput[1].value[1]!=2 && textInput[3].value[1]!=2 && textInput[1].value[1]!=6
	&& textInput[3].value[1]!=6 && textInput[1].value[1]!=9 && textInput[3].value[1]!=9
	&& textInput[1].value[1]!=0 && textInput[3].value[1]!=0){
		textButton[0].style.background = "linear-gradient(90deg,#deaaff,#ace4fd)";
...
window.onload = function () {
	...
	function num (n) {
		...
		$(textInput[n]).bind(bind_name,function () {
			...
			textInput[n].style.borderBottom = "1px solid #444";
			if(result && textInput[n].value != ""){
				if(textInput[1].value == textInput[3].value){
					...
					if(n==1 && textP[3].innerHTML=="家长电话与学生电话不能一致")
						textP[3].style.display = "none";
				}
				...
				if(textInput[n].value.toString().length != "" && (textInput[n].value[0]>=2 || textInput[n].value[0]==0
						|| textInput[n].value[1]==1 || textInput[n].value[1]==2 || textInput[n].value[1]==6 || textInput[n].value[1]==0 || textInput[n].value[1]==9)){
				textP[n].style.display = "block";
				textP[n].innerHTML =  "电话号码格式不正确";
				}
				else if(textInput[n].value.length<11){
					textP[n].innerHTML = "电话号码长度不足11位";
					textP[n].style.display = "block";
				}
				else if(textP[1].value!=textP[3].value){
					textP[n].style.display = "none";
				}
			}
 ...
 $("#city-picker").cityPicker({
	...
	$(textInput[2]).bind(bind_name,function () {
		...
		textInput[2].style.borderBottom = "1px solid #444";
		if(!result){
		...
		if(!result){
			...
			if(textInput[0].value == textInput[2].value){
				...
				if(textP[2].style.display != "家长姓名与学生姓名不能一致")
     			...
     			textP[0].style.color = "#b3b3b3";
				textP[0].innerHTML = textInput[0].value+"同学，您好，请核对您的信息是否有误: </br>学号: </br>班级: </br>毕业中学: </br>如信息错误请检查姓名是否输入有误;</br>如信息核对正确，请您完善您的个人相关信息;";
...
var certainId;
var studentName=function(textInput,department){
	$.ajax({
		url:'judge_stu_name',
		type:'POST',
		data:{
		...
		success:function(checkMess){ //message表示后台返回的数据
			...
			if(checkMess.status == "accept"){
				...
				if(checkMess.list==null){
					...
					textP[0].innerHTML =textInput+"同学，您好，请核对您的信息是否有误: </br>学号: "+checkMess.stuNo+"</br>班级: "+checkMess.classname+"</br>毕业中学: "+checkMess.highschool+"</br>如信息错误请检查姓名是否输入有误;</br>如信息核对正确，请您完善您的个人相关信息;";
				}
				else{
					...
					sameIdTd[2].style.background = "url(res/front/img/new_icon_locked.png) no-repeat center center";
					sameIdTd[2].style.backgroundSize = "1rem auto";
					for(var i=1;i<checkMess.list.length;i++){
						sameIdTd[3*(i+1)-1].style.background = "url(res/front/img/new_icon_unlock.png) no-repeat center center";
							...
							sameIdTd[3*(i+1)-1].onclick = function () {
								...
								for(var j=0;j<checkMess.list.length;j++){
									if(self!=sameIdTd[3*(j+1)-1]){
										sameIdTd[3*(j+1)-1].style.background = "url(res/front/img/new_icon_unlock.png) no-repeat center center";
										sameIdTd[3*(j+1)-1].style.backgroundSize = "1rem 1rem";
										
									}
									else{
										sameIdTd[3*(j+1)-1].style.background = "url(res/front/img/new_icon_locked.png) no-repeat center center";
										sameIdTd[3*(j+1)-1].style.backgroundSize = "1rem 1rem";	
					...
					sameIdBtn[1].onclick = function(){
						for(var i=0;i<checkMess.list.length;i++){
							if(sameIdTd[3*(i+1)-1].style.backgroundImage=="url(\"res/front/img/new_icon_locked.png\")"){
